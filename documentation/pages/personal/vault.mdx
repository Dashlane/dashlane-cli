...
dcli u url=someurl title=mytitle
# will return any entry for which the url matches someurl, or the title matches mytitle

dcli u url,title=mywebsite
# will return any entry for which either the url or the title matches mywebsite
```

Note: You can select a different output for login among `clipboard, login, json`. The JSON option outputs all the matching credentials.
```
