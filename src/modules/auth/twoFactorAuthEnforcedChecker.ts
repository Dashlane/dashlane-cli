...
export const twoFactorAuthEnforcedChecker = async (localConfiguration: LocalConfiguration, login: string) => {
    const remoteAuthentication: RemoteOrSSOAuthenticationType = hasEnvDeviceCredentials()
        ? RemoteOrSSOAuthenticationType.email_token
        : (
              await getRemoteAuthenticationAndSSOInfo({
                  login,
                  deviceAccessKey: localConfiguration.accessKey,
              })
    ...
    if (
        twoFAEnforced &&
        ['newDevice', 'login'].includes(twoFAEnforced) &&
        remoteAuthentication === RemoteOrSSOAuthenticationType.email_token
    ) {
        throw new Error(
