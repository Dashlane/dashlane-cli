...
export const CLI_VERSION: CliVersion = { major: 1, minor: 0, patch: 0 };
...
export const stringToCliVersion = (version: string): CliVersion => {
    ...
    if (versionSegments.length !== 3 || versionSegments.findIndex(isNaN) >= 0) {
        throw new Error(`This version string is invalid: ${version}`);
    }
